...
global crd_path
crd_path = [[0 for m in range(2*crdcount)] for n in range(2*crdcount)]
...
def max_chord(i,j):
    ...
    # case 2: k在裡面 ((k+1,j-1)+(i,k-1))+1 cf.case 1
        ...
        if s2 > s1:
            crd_path[i][j] = max_chord(k[j]+1,j-1) + max_chord(i,k[j]-1) + 1
            return s2
        else:
            crd_path[i][j] = max_chord(i,j-1)
            return s1
    ...
    # case 3: k = i (i+1,j-1)+1
        ...
        crd_path[i][j] = max_chord(i+1,j-1) + 1
...
def find_chord(i,j):
    ...
    # case 2: k在裡面 ((k+1,j-1)+(i,k-1))+1 cf.case 1
        ...
        if s2 > s1:
            cur += [[k[j],j]]
            return find_chord(k[j]+1,j-1) + find_chord(i,k[j]-1) +1
        else:
            return find_chord(i,j-1)
    # case 3: k = i (i+1,j-1)+1
    elif k[j] == i:
        cur += [[i,j]]
        return find_chord(i+1,j-1)
